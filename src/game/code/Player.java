...
public class Player extends BasicEntity {
    ...
    final Timer portalTimer, aniTimer;
    ...
    public Player(Vec2 pos) {
        ...
        Engine.lightList.add(new Light(pos, 3, "Player"));
        this.mass = -1;

        portalTimer = new Timer(3000, false);
        aniTimer = new Timer(100, true);
        aniTimer.start();
    ...
    public void update(Input input, double delta) {
        boolean up = input.getKeyEvent(KeyEvent.VK_W) || input.getKeyEvent(KeyEvent.VK_SPACE);
        boolean left = input.getKeyEvent(KeyEvent.VK_A);
        boolean right = input.getKeyEvent(KeyEvent.VK_S);
        ...
        if(!Collective.paused) {
            ...
            if (contains("Portal") && cooldown == 0 && portalTimer.isExpired()) {
                portalTimer.start();
                ...
                hitboxes.getFirst().setPos(new Vec2(entity.getPos().minus(new Vec2(8, 16))));
                cooldown = 3;
            }
        ...
        if(aniTimer.isExpired())
            Collective.aniIndex = Collective.aniIndex < aniLength - 1 ? Collective.aniIndex + 1 : 0;
    }
